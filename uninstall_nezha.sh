...
#
# Nezha All-in-One Smart Uninstaller v2.0
# 自动检测并彻底卸载 Nezha Dashboard 和 Agent (支持 Docker, systemd, init.d/procd)
#
...
uninstall_docker_components() {
    ...
    CONTAINERS=("nezha-dashboard" "nezha-agent")
    ...
    if [ "$containers_found" -eq 0 ]; then
        echo_info "  未发现 Nezha 相关的 Docker 容器。"
    fi
}
...
uninstall_systemd_components() {
    ...
    SERVICES=("nezha-dashboard.service" "nezha-agent.service")
    ...
    if [ "$services_found" -eq 1 ]; then
        ...
        echo_info "  未发现 Nezha 相关的 systemd 服务。"
    fi
}
...
uninstall_procd_components() {
    ...
    INIT_SCRIPTS=("nezha-dashboard" "nezha-agent")
    ...
    if [ "$scripts_found" -eq 0 ]; then
        echo_info "  未发现 Nezha 相关的 init.d 脚本。"
    fi
}
...
cleanup_filesystem_and_processes() {
    ...
    # 强制杀死所有可能残留的进程
    local pids=$(ps | grep 'nezha-agent' | grep -v 'grep' | awk '{print $1}')
    if [ -n "$pids" ]; then
        echo_warn "  发现残留的 nezha-agent 进程 (PID: $pids)，正在强制终止..."
        kill -9 $pids
    ...
    # 删除主程序目录
    if [ -d "/opt/nezha" ]; then
        echo_warn "  发现主程序目录: /opt/nezha"
        rm -rf "/opt/nezha"
        echo_success "    └─ 目录 '/opt/nezha' 已删除。"
    else
        echo_info "  未发现主程序目录 /opt/nezha。"
    fi

    # 清理安装脚本
    rm -f nezha.sh agent.sh
    echo_info "  已清理当前目录下的安装脚本。"
}
...
# 1. 检查 root 权限
if [ "$(id -u)" -ne 0 ]; then
   echo_error "此脚本需要以 root 权限运行。请使用 'sudo ./uninstall_nezha_pro.sh'"
   exit 1
fi
...
clear
echo_warn "========================================================"
echo_warn "         Nezha 监控通用智能卸载程序 v2.0"
echo_warn "========================================================"
echo ""
echo_info "本脚本将全面检测并永久删除本机所有的 Nezha 组件。包括："
echo_info " - Docker 容器 (dashboard/agent)"
echo_info " - Systemd 服务 (主流 Linux 系统)"
echo_info " - init.d 服务 (iStoreOS/OpenWrt 系统)"
echo_info " - /opt/nezha 等程序文件和目录"
echo_info " - 残留的进程"
echo ""
...
echo_info "请检查您的哪吒面板，确认此服务器是否已离线。"
echo ""
